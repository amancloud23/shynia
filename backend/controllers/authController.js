...
exports.updateAdminSettings = async (req, res) => {
  try {
    ...
    if (upi_id && !upi_id.match(/^[a-zA-Z0-9._-]+@[a-zA-Z0-9]+$/))
      return res.status(400).json({ message: 'Invalid UPI ID format (e.g., user@okhdfcbank)' });
    
    const user = await User.findByPk(req.user.id);
    await user.update({ upi_id: upi_id || null });
