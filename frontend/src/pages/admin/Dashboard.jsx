...
export default function Dashboard() {
  ...
  const cards = [
    ...
    { label: 'Banners',        value: '🎠',                         icon: '🎠', link: '/admin/banners' }
  ];
      ...
      <div className="container">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px', flexWrap: 'wrap', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            ...
            <Link to="/admin/settings" style={{ padding: '12px 28px', border: '1px solid #c9a96e', borderRadius: '4px', color: '#c9a96e', textDecoration: 'none', fontWeight: 500, fontSize: '14px', cursor: 'pointer' }}>
              ⚙️ Settings
            </Link>
          </div>
        </div>
              ...
              {recentOrders.map((order, i) => (
                <tr key={order.id} style={{ borderBottom: i < recentOrders.length - 1 ? '1px solid #ecddd8' : 'none' }}>
                  ...
                  <td style={{ padding: '14px 20px' }}>
                    <span style={{
                      background: order.status === 'delivered' ? '#e6fff0' : order.status === 'confirmed' || order.status === 'shipped' ? '#e6f5e6' : order.status === 'shipped' ? '#e6f0ff' : '#fff8e6',
                      color: order.status === 'delivered' ? '#1a7a40' : order.status === 'confirmed' ? '#1a7a40' : order.status === 'shipped' ? '#1a4db0' : '#b07800',
