...
const emptyForm = { name: '', description: '', price: 0, image_url: '', category: 'Gel', stock: 0, imageType: 'url', imageFile: null };
...
export default function ManageProducts() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.name || form.price === null || form.price === '' || !form.category) { toast.error('Fill required fields'); return; }
    if (form.imageType === 'upload' && !form.imageFile) { toast.error('Select an image file'); return; }
    if (form.imageType === 'url' && !form.image_url) { toast.error('Enter image URL'); return; }
    setLoading(true);
    try {
  ...
  const handleEdit = (p) => {
    setForm({ 
      ...
      imageType: p.image_url ? 'url' : 'upload',
      imageFile: null
    });
    setEditId(p.id);
    setShowForm(true);
  ...
  const inputStyle = {
    ...
    boxSizing: 'border-box', cursor: 'text', pointerEvents: 'auto'
  };
      ...
      <div className="container">
        ...
        {showForm && (
          <div style={{ background: '#fff', border: '1px solid #ecddd8', borderRadius: '4px', padding: '32px', marginBottom: '36px' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }} noValidate>
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <input style={inputStyle} type="text" name="name" value={form.name} onChange={handleChange} placeholder="e.g. Rose Petal Dreams" />
              </div>
              <div>
                ...
                <input style={inputStyle} name="price" type="number" step="0.01" min="0" value={form.price} onChange={handleChange} placeholder="19.99" />
              </div>
              <div>
              ...
              <div>
                ...
                <input style={inputStyle} name="stock" type="number" min="0" value={form.stock} onChange={handleChange} placeholder="0" />
              </div>
              <div style={{ gridColumn: '1 / -1' }}>
                <label style={{ fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#9e7b74', display: 'block', marginBottom: '10px' }}>Image Source</label>
                <div style={{ display: 'flex', gap: '20px', marginBottom: '10px' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '14px', color: '#3d2b27', cursor: 'pointer' }}>
                    <input type="radio" name="imageType" value="url" checked={form.imageType === 'url'} onChange={handleChange} />
                    Image URL
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '14px', color: '#3d2b27', cursor: 'pointer' }}>
                    <input type="radio" name="imageType" value="upload" checked={form.imageType === 'upload'} onChange={handleChange} />
                    Upload Image
                  </label>
                </div>
                {form.imageType === 'url' ? (
                  <input style={inputStyle} type="url" name="image_url" value={form.image_url} onChange={handleChange} placeholder="https://..." />
                ) : (
                  <input style={inputStyle} type="file" name="imageFile" accept="image/*" onChange={handleChange} />
                )}
              </div>
              <div style={{ gridColumn: '1 / -1' }}>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                <button type="submit" disabled={loading} className="btn-primary" style={{ padding: '12px 36px', opacity: loading ? 0.7 : 1, cursor: 'pointer', pointerEvents: 'auto' }}>
